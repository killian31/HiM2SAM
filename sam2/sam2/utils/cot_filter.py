...
class torch_GaussianKDE:
    ...
    def dynamic_bandwidth(self, density, min_bandwidth=1.0, max_bandwidth=5.0):
        ...
        normalized_density = (density - density_min) / (
            density_max - density_min + 1e-6
        )
        bandwidths = max_bandwidth - normalized_density * (
    ...
    def evaluate(self, points):
        ...
        if self.bandwidth is None:
            ...
            bandwidths = self.dynamic_bandwidth(
                density, min_bandwidth=1.0, max_bandwidth=9.0
            )
        else:
        ...
        kernel_vals = (
            kernel_vals / (math.sqrt(2 * math.pi) * bandwidths) ** self.n_features
        )
...
class RVCotFilter:
    def __init__(
        ...
        **args
        ...
        self.cot_model = torch.hub.load(
            "co-tracker", "cotracker3_offline", source="local"
        )
        ...
        self.grid_points = torch.stack(
            [grid_y.flatten(), grid_x.flatten()], dim=1
        ).cpu()  # (N, 2)
        self.ct = 0
    ...
    def predict(
        ...
        inference_state,
        cur_masks,
        iou_aggregation_method="mean",
        debug_flag=False,
        sample_count=100,
    ):
        # merge the cur_masks, reverse cot tracking, return iou for per historical frame
        assert iou_aggregation_method in [
            "farest",
            "mean",
        ]  # farest: the farest frame from current frame
        ...
        feed_cot_video_frames = torch.stack(
            [
                inference_state["cot_cache_frames"][i][0]
                for i in range(frame_idx - 1, frame_far - 1, -1)
            ],
            dim=0,
        )
        if self.mirrow_padding:
            reverse_frames = feed_cot_video_frames.flip(dims=[0])[1:, :, :]
            feed_cot_video_frames = torch.cat(
                [feed_cot_video_frames, reverse_frames], dim=0
            )
        feed_cot_video_frames = (
            feed_cot_video_frames * img_std + img_mean
        )  # reverse to 0-1
        ...
        points = self.fps_sample_from_mask(
            union_mask[0], sample_count, errod=True, kernel=k, sparse=sparse
        )
        if points is None:
        ...
        x.clamp_(0, 1023)
        y.clamp_(0, 1023)
        points_mask_mapping = torch.stack(
            [
                cur_masks_bin[0][0][y, x],
                cur_masks_bin[1][0][y, x],
                cur_masks_bin[2][0][y, x],
            ],
            dim=1,
        )
        # reverse track sampled points
        pred_tracks, pred_visibility = self.cot_model(
            feed_cot_video_frames.unsqueeze(0).to(dtype=torch.float32),
        ...
        for f in range(frame_idx - 1, frame_far - 1, -1):
            if f == 0:
                his_mask = inference_state["output_dict"]["cond_frame_outputs"][0][
                    "pred_masks"
                ]
            else:
                his_mask = inference_state["output_dict"]["non_cond_frame_outputs"][f][
                    "pred_masks"
                ]
        ...
        cot_iou_score = self.iou_score_point_mask(
            pred_tracks,
            pred_visibility,
            historical_masks,
            points_mask_mapping,
    ...
    def prepare_masks(self, masks):
        ...
        for i in range(N):
            ...
            binary_mask = (cur_masks > 0.0).int()
            masks_list.append(binary_mask)
        return masks_list

    def fps_sample_from_mask(
        self, binary_mask, sample_count=50, errod=True, kernel=None, sparse=False
    ):
        if kernel is None:
            k = 7
        else:
        ...
        if errod:
            binary_mask = F.max_pool2d(
                -binary_mask[None][None].to(torch.float32),
                kernel_size=k,
                stride=1,
                padding=k // 2,
            )[0][0]
        ...
        fps_points, _ = self.sample_farthest_points_naive(
            points=points[None], K=sample_count
        )
    ...
    def sample_farthest_points_naive(
        ...
        for n in range(N):
            ...
            for i in range(1, k_n):
                ...
                closest_dists = torch.min(
                    dist_to_last_selected, closest_dists
                )  # (P - i)
    ...
    def iou_score_point_mask(
        ...
        for i in range(points_mask_mapping.shape[1]):  # 3 proposals
            ...
            for t in range(len(historical_masks)):
                validpoints = (
                    pred_visibility[0, t, :] & points_mask_mapping[:, i]
                )  # viliad flag at time t, proposal i-th
                if validpoints.sum() < 10:
                    ...
                    if not self.mirrow_padding:
                        ...
                        cur_pred_tracks = pred_tracks[:, -1 - t, :, :][
                            validpoints.bool()
                        ]
                    conf_mask = self.torch_gaussian_kernel(
                        cur_pred_tracks, bandwidth=bw
                    )
                cur_ious.append(
                    self.nonbinary_iou(conf_mask, historical_masks[t][0][0])
                )
